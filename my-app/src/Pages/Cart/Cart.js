      ...
      <div className="cart_window">
        <Hide breakpoint="(max-width: 1136px)">
              ...
              <Tbody>
                {cart_store.cartData.map((el) => (
                  <Tr className="Cart_table_row">
                    <Cart_row
                      cartItemName={el.name}
                      hendlecartUpdate={(val) => hendlecartUpdate(val, el)}
                      cartItemPrice={el.price}
                      cartItemQuantity={el.qty}
                      cartItemTotal={el.qty * el.price}
                      hendleCartClickRemove_itrm={() =>
                        hendleCartClickRemove_itrm(el)
                      }
                    />
                  </Tr>
                ))}
          ...
          <div className="cart_total_card">
            <div className="cart_box1_button1">
              <span Style="display:flex;justify-content:start">
                <ContinueShopping
                  cartbuttonname={"Cler Cart"}
                  hendleCartbutton={hendleClearCart}
                />
              </span>
            </div>

            <div className="cart_box1_button2">
              <span Style="display:flex;justify-content:start">
          ...
          <div className="cart_min_box">
            {cart_store.cartData.map((el) => (
              <>
                <Cart_item_Min
                  item_image={el.image}
                  item_discription={el.name}
                  item_price={el.price}
                  Item_Quantity={el.qty}
                  hendlecartUpdate={(val) => hendlecartUpdate(val, el)}
                  hendleCartClickRemove_itrm={() =>
                    hendleCartClickRemove_itrm(el)
                  }
          ...
          <div Style="background:#37474f;height:20%;display:flex;justify-content:space-around;align-items: center;">
            <div>
              <Text>
                ...
                <br />
                <b Style="color:white">RS {cart_store.subTotal.toFixed(2)}</b>
              </Text>
            </div>
            <div>
              <Link to="/Checkout">
                <Button Style="background:#e95f62;color:white">Checkout</Button>
              </Link>
            </div>
          </div>
        </div>
